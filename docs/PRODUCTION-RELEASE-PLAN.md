...
>
> **Last assessed:** 2026-08-23, against commit `12a9e52` (`fix/react-code-review-critical-issues`). Re-assess the “current-state facts” after significant architecture changes.
...
3. The app is deliberately excluded from the pnpm workspace. Root CI, root typecheck, root test, and root build do not test it. Its independent dependencies were absent during this assessment, so `pnpm --dir apps/editor test` fails before tests run with `ERR_MODULE_NOT_FOUND` for `@vitejs/plugin-react`. Its lint and typecheck pass only once its own dependencies are installed.
...
6. There is no persisted editor-project/media strategy, autosave, recovery UX, runtime error boundary, analytics/error reporting, browser capability screen, release checklist, or production E2E suite.
...
- `pnpm --dir apps/editor lint` and `typecheck` passed in the available installation.
- `pnpm --dir apps/editor test` could not start because the standalone app’s dependencies were not installed. Its excluded-workspace model is therefore working as designed, but it means editor verification is missing from the actual release gate.
- There is Playwright in root dev dependencies, but no checked-in browser-test configuration or editor E2E test suite.
...
- [ ] Fresh-clone install works for workspace and `apps/editor` independently.
- [ ] `pnpm run ci` passes.
- [ ] `pnpm run editor:verify` passes and is required in CI.
- [ ] Root package build/typecheck/test and standalone editor build/typecheck/test are green with zero unexpected warnings.
- [ ] Playwright runs the production editor build on all supported browser targets.
...
# Standalone editor—these should be formalized as root scripts in P0
