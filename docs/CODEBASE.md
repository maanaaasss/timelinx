...
9. [Design Principles & Invariants](#9-design-principles--invariants)
...
type CompressionPolicy =
  { readonly kind: 'none' } | { readonly kind: 'last-write-wins'; readonly windowMs: number };
...
- **DUPLICATE\_*_ID** — no duplicate effect/keyframe IDs
...
```typescript
type SnapPointType =
  'ClipStart' | 'ClipEnd' | 'Playhead' | 'Marker' | 'InPoint' | 'OutPoint' | 'BeatGrid';
...
import {
  assertInvariants,
  assertInvariantViolations,
  applyAndAssert,
} from './helpers/assertInvariants';
...
import {
  ...
  buildTrack,
  buildAsset,
  buildMultiTrackState,
} from './helpers/buildState';

const state = buildState({
  tracks: [buildTrack('v1', 'video', [buildClip('c1', { start: 0, end: 200 })])],
});
const multiState = buildMultiTrackState({
  tracks: [{ id: 'v1', type: 'video', clips: [{ id: 'c1', start: 0, end: 200 }] }],
});
```
...
fc.assert(
  fc.property(arbitraryOperation(state), (op) => {
    /* ... */
  }),
);
...
fc.assert(
  fc.property(arbitraryInitialState(), (state) => {
    /* ... */
  }),
);
