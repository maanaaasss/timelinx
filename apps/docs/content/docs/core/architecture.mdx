...
description: Package boundaries, data flow, and runtime responsibilities inside @timelinx/core
---

`@timelinx/core` is designed as a deterministic editing kernel. It stores timeline state in plain TypeScript data, accepts edit intent as transactions, validates the result, and returns a new state. That separation lets the same model power a custom canvas, a React editor, server-side import/export jobs, tests, and future collaboration layers.

The core rule: do not mutate tracks, clips, markers, or asset registries directly in application code. Build a `Transaction`, call `dispatch()`, and render the returned `nextState` only when the transaction is accepted.

## Package Boundary

| Area | Responsibility |
|------|----------------|
| Types | `TimelineState`, `Timeline`, `Track`, `Clip`, `Asset`, IDs, `TimelineFrame`, `OperationPrimitive`, and timeline metadata types. |
| Engine | Dispatch, apply, history, playback, timeline engine wrapper, frame resolver, serializers, import/export helpers, indexes, and virtual windows. |
| Validation | `validateOperation()` before each primitive and `checkInvariants()` after the proposed state is built. |
| Tools | Pointer/key editing tools that produce transactions and provisional drag state. |
| Systems | Queries, asset registry helpers, and reusable validation helpers. |

The boundary is intentionally narrow. `core` can understand editing concepts such as snapping, ripple movement, frame resolution, keyframes, captions, markers, and playback contracts. It cannot understand DOM events, React components, CSS, browser layout, or app-specific storage.
