...
<Callout title="Prefer DELETE + INSERT for compound edits">
  `SLICE_CLIP` is a convenience operation for single-frame splits. For a razor tool that must
  preview ghost clips or handle undo as one action across multiple tracks, use `DELETE_CLIP` +
  `INSERT_CLIP` pairs in a transaction.
</Callout>
