...
<Callout title="Track ordering">
  Tracks are stored in the `tracks` array. The first entry (index 0) renders at the top of the
  timeline. Use `REORDER_TRACK` to change positions.
</Callout>
...
<Callout type="warn" title="Asset must be registered first">
  `INSERT_CLIP` will reject with `ASSET_MISSING` if the clip's `assetId` is not already in
  `assetRegistry`. Always dispatch `REGISTER_ASSET` in the same transaction or a prior one.
</Callout>
