...
<Callout type="info">
  TypeScript is not required, but strongly recommended. All packages are written in TypeScript with
...
<Accordions>
  <Accordion id="headless" title="Can I use only Core without React?">
    Yes. Core is headless - no React, no DOM. Use it for server-side processing, tests, or with any other UI framework.
  </Accordion>
