...
<Callout type="warn" title="Firefox and WebCodecs">
  WebCodecs is not available in Firefox. Use `isSupported()` on `WebCodecsDecoderAdapter` before
  passing it to the pipeline. Without a video decoder, you can still use the editor for editing -
