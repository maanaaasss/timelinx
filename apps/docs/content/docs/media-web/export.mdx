...
const result = await exporter.export(state, pipeline, config, (progress) => {
  console.log(
    `${Math.round(progress.progress * 100)}%  - ${progress.framesEncoded}/${progress.totalFrames} frames`,
  );
});
```
...
<Callout type="warn" title="captureStream() is Chromium-only">
  `SimpleExportAdapter` uses `canvas.captureStream()` to feed the compositor output into
  `MediaRecorder`. This API is Chromium-only and is not available in Safari. On Safari, the export
  button should be hidden or replaced with a server-side export flow.
</Callout>

<Callout type="warn" title="MP4 container support varies">
  `MediaRecorder` with `'video/mp4'` is only guaranteed in Chrome 130+. For cross-browser
  compatibility, export as WebM and convert server-side with FFmpeg if needed.
</Callout>
