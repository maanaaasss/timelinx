...
<Callout title="State comes from the engine">
  The `onSave` callback receives the current state from `engine.getSnapshot().state`. If you
  dispatch further changes after this call, the save will be stale. Always call `serializeProject`
  inside the `onSave` callback, not outside it.
</Callout>
