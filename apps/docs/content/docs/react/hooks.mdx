...
<Callout type="warn" title="Mount sparingly">
  `usePlayheadFrame` re-renders on every animation frame during playback (30–60× per second). Keep
  it only in components that display the playhead position - ruler, transport, and compositor. Do
  not use it in clip components.
</Callout>
