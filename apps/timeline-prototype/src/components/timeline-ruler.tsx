...
function getLabelInterval(ppf: number, fps: number): number {
  if (ppf < 5) return Math.round(30 * fps);
  if (ppf < 15) return Math.round(10 * fps);
  if (ppf < 40) return Math.round(5 * fps);
...
export function TimelineRuler({
  ...
  const majorIntervalFrames = getLabelInterval(ppf, fps);
  ...
  const draw = useCallback(() => {
    ...
    // Major ticks + labels (adaptive interval)
    ...
    for (let f = firstMajor; f <= endFrame; f += majorIntervalFrames) {
      ...
      const seconds = f / fps;
      ctx.fillText(formatTimecode(seconds), x + 3, 2);
    }
  ...
  return (
    <div ref={containerRef} className="timeline-ruler">
