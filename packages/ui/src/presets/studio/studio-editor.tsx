/**
 * StudioEditor — flagship timeline editor component.
 *
 * Composes all Studio preset components into a clean, professional
 * timeline editor. Owns all layout coordination, pointer/keyboard
 * handling, and scroll sync.
 *
 * Usage:
...
import { StudioTrack } from './studio-track';
import { StudioClip } from './studio-clip';
import { StudioPlayhead } from './studio-playhead';
import { IconPlus, IconFilm, IconHeadphones } from './icons';

const DEFAULT_TRACK_HEIGHT_VIDEO = 68;
const DEFAULT_TRACK_HEIGHT_AUDIO = 56;
...
const ClipRow = React.memo(function ClipRow({
  ...
  return (
    ...
    >
      {clips.length === 0 && (
        <div
          className="tl-track-empty-scanlines"
          style={{
            position: 'absolute',
            inset: 0,
            display: 'flex',
...
function EditorInner({
  ...
  return (
    ...
    >
      <StudioToolbar />
      <StudioRuler contentRef={rulerContentRef} />
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        >
          ...
          >
            ...
            <StudioPlayhead totalHeight={totalTrackHeight} topOffset={24} />
          </div>
        </div>
      </div>
    </div>
