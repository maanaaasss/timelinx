...
export class TimelineEngine {
  ...
  addClip(
    ...
    const result = legacyDispatch(this.history, (state) => ClipOps.addClip(state, trackId, clip));
    if (result.accepted && result.history) {
      this.history = result.history;
      this.notify();
    }
  ...
  removeClip(clipId: string): { accepted: boolean; errors?: { code: string; message: string }[] } {
    const result = legacyDispatch(this.history, (state) => ClipOps.removeClip(state, clipId));
    if (result.accepted && result.history) {
      this.history = result.history;
      this.notify();
    }
  ...
  trimClip(
    clipId: string,
    newMediaIn: TimelineFrame,
    newMediaOut: TimelineFrame,
  ): { accepted: boolean; errors?: { code: string; message: string }[] } {
    const result = legacyDispatch(this.history, (state) =>
  ...
  addTrack(track: Track): { accepted: boolean; errors?: { code: string; message: string }[] } {
    const result = legacyDispatch(this.history, (state) => TrackOps.addTrack(state, track));
    if (result.accepted && result.history) {
      this.history = result.history;
      this.notify();
    }
  ...
  removeTrack(trackId: string): {
    ...
    const result = legacyDispatch(this.history, (state) => TrackOps.removeTrack(state, trackId));
    if (result.accepted && result.history) {
      this.history = result.history;
      this.notify();
    }
