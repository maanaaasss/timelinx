 ...
 */
export function findMarkersByColor(state: TimelineState, color: string): Marker[] {
 ...
 */
export function findMarkersByLabel(state: TimelineState, label: string): Marker[] {
  const lower = label.toLowerCase();
  return state.timeline.markers.filter((m) => m.label.toLowerCase().includes(lower));
}
