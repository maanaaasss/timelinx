...
function makeState({
  ...
  const clipLeft = createClip({
    ...
    mediaIn: toFrame(leftMediaIn),
    mediaOut: toFrame(leftMediaOut),
  });
  const clipRight = createClip({
    id: 'clip-right',
    assetId: 'asset-1',
    trackId: 'track-1',
    timelineStart: toFrame(rightStart + gap),
    timelineEnd: toFrame(rightStart + gap + 200),
    mediaIn: toFrame(rightMediaIn),
    mediaOut: toFrame(rightMediaOut),
  });
  const track = createTrack({
    id: 'track-1',
    name: 'V1',
    type: 'video',
    clips: [clipLeft, clipRight],
  });
...
describe('RollTrimTool — roll RIGHT: left shorter, right longer', () => {
  ...
  it('combined duration unchanged after roll right', () => {
    ...
    const origCombined =
      origLeft.timelineEnd -
      origLeft.timelineStart +
      (origRight.timelineEnd - origRight.timelineStart);
    const newCombined =
      newLeft.timelineEnd - newLeft.timelineStart + (newRight.timelineEnd - newRight.timelineStart);
...
describe('RollTrimTool — clamp: right media bound (E leftward)', () => {
  it('cannot roll left past right clip mediaIn=0 (constraint E)', () => {
    ...
    const clipRight = createClip({
      ...
      mediaIn: toFrame(50),
      mediaOut: toFrame(250), // duration=200, mediaIn=50
    });
    const track = createTrack({
      id: 'track-1',
      name: 'V1',
    ...
    const state = createTimelineState({
      timeline: tl,
      assetRegistry: new Map([[ASSET_ID, makeAsset()]]),
    });
...
describe('RollTrimTool — clamp: min-duration (1 frame)', () => {
  it('left clip cannot be rolled to 0 frames — clamped to frame 1 (constraint A)', () => {
    ...
    const clipRight = createClip({
      ...
      timelineEnd: toFrame(400),
      mediaIn: toFrame(199),
      mediaOut: toFrame(399), // mediaIn=199 → E: 200-199=1 = A
    });
    const track = createTrack({
      id: 'track-1',
      name: 'V1',
