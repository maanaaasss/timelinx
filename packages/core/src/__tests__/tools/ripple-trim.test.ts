...
function makeEndTrimState({
  aMediaIn = 0,
  aMediaOut = 200,
}: { aMediaIn?: number; aMediaOut?: number } = {}): TimelineState {
  ...
  const clipA = createClip({
    ...
    mediaIn: toFrame(aMediaIn),
    mediaOut: toFrame(aMediaOut),
  });
...
describe('RippleTrimTool — END trim: -delta (clip gets shorter)', () => {
  ...
  beforeEach(() => {
    tool = new RippleTrimTool();
    state = makeEndTrimState();
  });
  ...
  it('left clips shift LEFT by -delta (-50), bounded by frame-0 clamp', () => {
    ...
    const stateWithRoom = buildState([
      ...
      createClip({
        ...
        timelineEnd: toFrame(400),
        mediaIn: toFrame(100),
        mediaOut: toFrame(300), // 100 frames of leftward headroom
      }),
    ]);
  ...
  it('partial clamp: allows trim up to where leftmost clip hits frame 0', () => {
    ...
    const state2 = buildState([
      ...
      createClip({
        ...
        timelineEnd: toFrame(400),
        mediaIn: toFrame(120),
        mediaOut: toFrame(320), // 120 frames leftward headroom
      }),
    ]);
