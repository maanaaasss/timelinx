...
function clickTarget(
  tool: RippleDeleteTool,
  state: TimelineState,
): ReturnType<RippleDeleteTool['onPointerUp']> {
...
function applyAndCheck(
  state: TimelineState,
  tx: ReturnType<RippleDeleteTool['onPointerUp']>,
): TimelineState {
...
describe('RippleDeleteTool — defensive: clip missing at onPointerUp', () => {
  it('pendingClipId set but clip not in ctx.state → null', () => {
    ...
    const ghostState = createTimelineState({
      timeline: tl,
