...
describe('Coverage: validators.ts — remaining branches', () => {
  ...
  it('SET_CLIP_METADATA on non-existent clip is rejected', () => {
    const state = makeState();
    const result = dispatch(
      state,
      makeTx('Set Meta', [
  ...
  it('SET_CLIP_METADATA on valid clip succeeds', () => {
    const state = makeState();
    const result = dispatch(
      state,
      makeTx('Set Meta', [
  ...
  it('SET_CLIP_METADATA with invalid metadata is rejected', () => {
    const state = makeState();
    const result = dispatch(
      state,
      makeTx('Set Meta', [
    ...
    expect(result.accepted).toBe(false);
    if (!result.accepted) expect(result.reason).toBe('INVARIANT_VIOLATED');
  });
});
