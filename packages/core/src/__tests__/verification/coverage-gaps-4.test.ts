...
describe('Coverage: validators.ts — remaining branches', () => {
  ...
  it('ADD_MARKER point with frame < 0 is rejected', () => {
    ...
    const result = dispatch(
      ...
      makeTx('Add Marker', [
        {
          type: 'ADD_MARKER',
          marker: { id: 'm1', type: 'point', frame: toFrame(-10) },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
  ...
  it('ADD_MARKER point with frame > duration is rejected', () => {
    ...
    const result = dispatch(
      ...
      makeTx('Add Marker', [
        {
          type: 'ADD_MARKER',
          marker: { id: 'm1', type: 'point', frame: toFrame(10000) },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
  ...
  it('ADD_BEAT_GRID with existing beat grid is rejected', () => {
    ...
    const result = dispatch(
      s1.nextState,
      makeTx('Add Beat Again', [
        {
          type: 'ADD_BEAT_GRID',
          beatGrid: { bpm: 140, timeSignature: [4, 4], offset: toFrame(0) },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
  ...
  it('ADD_BEAT_GRID with invalid time signature [0,4] is rejected', () => {
    ...
    const result = dispatch(
      ...
      makeTx('Add Beat', [
        {
          type: 'ADD_BEAT_GRID',
          beatGrid: { bpm: 120, timeSignature: [0, 4], offset: toFrame(0) },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
  ...
  it('ADD_BEAT_GRID with negative time signature [4,-1] is rejected', () => {
    ...
    const result = dispatch(
      ...
      makeTx('Add Beat', [
        {
          type: 'ADD_BEAT_GRID',
          beatGrid: { bpm: 120, timeSignature: [4, -1], offset: toFrame(0) },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
      ...
      makeTx('Move', [
        {
          type: 'MOVE_CLIP',
          clipId: toClipId('clip-a'),
          targetTrackId: toTrackId('nonexistent'),
      ...
      makeTx('Link', [
        {
          ...
          linkGroup: {
            id: 'lg-1',
            name: 'Link',
            clipIds: [toClipId('clip-a'), toClipId('nonexistent')],
          },
        },
      ]),
    );
      ...
      makeTx('Link', [
        {
          ...
          linkGroup: {
            id: 'lg-1',
            name: 'Link',
            clipIds: [toClipId('clip-a'), toClipId('clip-b')],
          },
        },
      ]),
    );
      ...
      makeTx('Insert Gen', [
        {
          ...
          trackId: toTrackId('nonexistent'),
          atFrame: toFrame(0),
          generator: { type: 'text', duration: toFrame(100), text: 'T' },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
      ...
      makeTx('Insert Gen', [
        {
          type: 'INSERT_GENERATOR',
          trackId: toTrackId('track-1'),
          atFrame: toFrame(8900),
          generator: { type: 'text', duration: toFrame(200), text: 'T' },
        },
      ]),
    );
    expect(result.accepted).toBe(false);
