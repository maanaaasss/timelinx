...
export function validateClip(state: TimelineState, clip: Clip): ValidationResult {
  ...
  if (!asset) {
    errors.push(
      invalidResult('ASSET_NOT_FOUND', `Asset '${clip.assetId}' not found in registry`, {
        clipId: clip.id,
  ...
  if (clip.mediaIn < 0) {
    errors.push(
      invalidResult('INVALID_MEDIA_IN', `Clip media in (${clip.mediaIn}) must be >= 0`, {
        clipId: clip.id,
...
export function validateTimeline(state: TimelineState): ValidationResult {
  ...
  if (state.timeline.fps <= 0) {
    errors.push(
      invalidResult('INVALID_FPS', `Timeline FPS must be positive, got ${state.timeline.fps}`, {
...
export function validateTrackTypeMatch(
  ...
  if (!asset) {
    return invalidResult('ASSET_NOT_FOUND', `Asset '${clip.assetId}' not found in registry`, {
      clipId: clip.id,
      assetId: clip.assetId,
    });
  }
